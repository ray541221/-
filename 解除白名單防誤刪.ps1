...
function Remove-DenyDelete($Path, [bool]$Recursive) {
    ...
    if (-not $item.PSIsContainer) { Set-ItemProperty -LiteralPath $Path -Name IsReadOnly -Value $false }
}
Remove-DenyDelete $Root $false
foreach ($name in $WhiteList) { Remove-DenyDelete (Join-Path $Root $name) $true }
Write-Host "完成：Codex 根目錄與白名單防誤刪已解除。"
